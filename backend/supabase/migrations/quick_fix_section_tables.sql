-- Quick fix: Create section configuration and allocation tables
...
-- Add automatic timestamp triggers (safe version)
DO $$
BEGIN
    -- Create trigger for section_configurations
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_section_configurations_modtime') THEN
        CREATE TRIGGER update_section_configurations_modtime 
            BEFORE UPDATE ON section_configurations 
            FOR EACH ROW EXECUTE FUNCTION update_modified_column();
    END IF;
    
    -- Create trigger for section_allocations
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_section_allocations_modtime') THEN
        CREATE TRIGGER update_section_allocations_modtime 
            BEFORE UPDATE ON section_allocations 
            FOR EACH ROW EXECUTE FUNCTION update_modified_column();
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        -- If triggers fail, it's not critical for basic functionality
